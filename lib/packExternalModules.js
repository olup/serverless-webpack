 ...
 * @this - The active plugin instance
 */
function getProdModules(externalModules, packagePath, dependencyGraph, forceExcludes, externalPackagesDependencies) {
  ...
  _.forEach(externalModules, module => {
    ...
    if (moduleVersion) {
      ...
      if (!packageJson.devDependencies || !packageJson.devDependencies[module.external]) {
        ...
        // add dependencies versions from external packages, if provided
        if (!moduleVersion && externalPackagesDependencies) {
          for (const dep of externalPackagesDependencies) {
            if (dep[module.external]) {
              moduleVersion = dep[module.external];
              this.serverless.cli.log(`INFO: Found external version for ${module.external}`);
              break;
            }
          }
        }
...
module.exports = {
  ...
  packExternalModules() {
    ...
    // custom conf
    const externalPackagePaths = includes.externalPackagePaths || [];
    const packageJsonPath = path.join(process.cwd(), packagePath);
    ...
    return BbPromise.try(() => Packagers.get.call(this, this.configuration.packager)).then(packager => {
      ...
      const externalPackagesDependencies = externalPackagePaths.map(p => {
        const extPackage = this.serverless.utils.readFileSync(path.join(process.cwd(), p));
        return extPackage.dependencies;
      });
